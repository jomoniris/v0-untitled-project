...
export async function GET() {
  try {
    ...
    const finalCountResult = await sql`SELECT COUNT(*) as count FROM vehicle_groups`
    const finalRecordCount = finalCountResult[0]?.count

    return NextResponse.json({
      success: true,
      message: tableExists ? "Vehicle groups table already exists" : "Vehicle groups table created",
      initialRecordCount: recordCount,
      finalRecordCount,
      sampleDataAdded: recordCount === 0 && finalRecordCount > 0,
    })
  } catch (error) {
    console.error("Error setting up vehicle groups:", error)
    return NextResponse.json(
      {
        success: false,
        message: "Error setting up vehicle groups",
        error: error.message,
      },
      { status: 500 },
    )
